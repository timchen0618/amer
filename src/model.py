...
class EmbeddingModelSSPredLength(EmbeddingModel):
    ...
    def generate(
        ...
        **inputs
        ...
        if 'input_ids' in inputs:
            outputs = self.base_causallm.generate(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'], max_new_tokens=NUM_LENGTH_LABELS, do_sample=False, output_hidden_states=True)
        else:
            outputs = self.base_causallm.generate(inputs_embeds=self.input_projection(inputs['inputs_embeds'].float()), attention_mask=inputs['attention_mask'], max_new_tokens=NUM_LENGTH_LABELS, do_sample=False, output_hidden_states=True)
...
class EmbeddingModelSSVariableLeftPadDocEncTrained(EmbeddingModel):
    ...
    def __init__(
        ...
        super(EmbeddingModelSSVariableLeftPadDocEncTrained, self).__init__(
            base_causallm=base_causallm,
            start_latent_id=start_latent_id,
            eos_token_id=eos_token_id,
            embedding_model_dim=embedding_model_dim,
    ...
    def last_token_pool(self, last_hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        if left_padding:
            ...
            print('arrange', torch.arange(batch_size, device=last_hidden_states.device))
    ...
    def forward(self, **inputs):
        ...
        ########### Unfreeze the Document Encoder ###########
        # get the document embeddings
        print('inputs input document ids', inputs['input_document_ids'].size(), 'inputs attention mask document', inputs['attention_mask_document'].size())
        
        reshaped_mask = inputs['attention_mask_document'].reshape(-1, inputs['attention_mask_document'].size(-1))
        print('reshaped size', inputs['input_document_ids'].reshape(-1, inputs['input_document_ids'].size(-1)).size(), reshaped_mask.size())
        outputs = self.base_causallm(input_ids=inputs['input_document_ids'].reshape(-1, inputs['input_document_ids'].size(-1)),
                                     attention_mask=reshaped_mask,
                                     output_hidden_states=True)
        doc_embeddings = self.last_token_pool(outputs.hidden_states[-1], reshaped_mask)  # (bsz * 2 * output_len, hidden_size)
        # print('doc_embeddings', doc_embeddings.size())
        batch_size = inputs['input_ids'].size(0)
        # split the doc embeddings into positive and negative embeddings (half for each)
        assert doc_embeddings.size(0) % 2 == 0, (doc_embeddings.size(0))
        labels = doc_embeddings[:doc_embeddings.size(0)//2]  # (bsz * output_len, hidden_size)
        negative_embeddings = doc_embeddings[doc_embeddings.size(0)//2:]  # (bsz * output_len, hidden_size)
        # reshape the labels and negative embeddings to (bsz, output_len, hidden_size). Use reshape instead of view to avoid the warning.
        labels = labels.reshape(batch_size, -1, labels.size(-1))
        negative_embeddings = negative_embeddings.reshape(batch_size, -1, negative_embeddings.size(-1))
        # print('labels', labels.shape, 'negative_embeddings', negative_embeddings.shape)

        # get the number of output query embeddings
        output_len = labels[0].size(0)
        for i in range(labels.size(0)):
            assert labels[i].size(0) == output_len, (labels[i].size(0), output_len)
        ########### Unfreeze the Document Encoder ###########
...
class EmbeddingModelSSVariableLeftPadPredLength(EmbeddingModel):
    ...
    def __init__(
        ...
        super(EmbeddingModelSSVariableLeftPadPredLength, self).__init__(
            base_causallm=base_causallm,
            start_latent_id=start_latent_id,
            eos_token_id=eos_token_id,
            embedding_model_dim=embedding_model_dim,
    ...
    def generate(
        ...
        **inputs
        ...
        # input_embeds -> (1, query_length, 1024)
        # attention mask -> (1, query_length)
        ...
        if 'input_ids' not in inputs:
            # get the query input embeddings
        ...
        inputs['attention_mask'] = torch.cat((inputs['attention_mask'], torch.ones(inputs['attention_mask'].size(0), NUM_LENGTH_LABELS).to(inputs['attention_mask'].device)), dim=1)
         
        
        # if there are input ids, we can just concatenate the input_ids and the length input ids 
        if 'input_ids' in inputs:
            new_input_ids = torch.cat((inputs['input_ids'], outputs), dim=1)
            outputs = self.base_causallm(input_ids=new_input_ids, attention_mask=inputs['attention_mask'], output_hidden_states=True)
        else: # if not, need to combine the input_embeds of the query and the length labels
            raise NotImplementedError("Not implemented")
            outputs = self.base_causallm(input_ids=outputs, attention_mask=torch.ones(inputs['attention_mask'].size(0), NUM_LENGTH_LABELS).to(inputs['attention_mask'].device), output_hidden_states=True)
            length_labels_hidden_states = outputs.hidden_states[0].clone()  # (bsz, 4, 2048)
...
def load_model(train_lora, base_model_id, adapter_path, linear_checkpoint_path, embedding_model_dim, 
    ...
    elif model_type == "EmbeddingModelSSVariableLeftPadDocEncTrained":
        model_class = EmbeddingModelSSVariableLeftPadDocEncTrained
    else:
