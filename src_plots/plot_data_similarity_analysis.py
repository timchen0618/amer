...
data = {
    'AmbigQA': {
        'L2': [0.91, 0.42, 0.68],
        'Cosine': [0.59, 0.90, 0.77]
    },
    'QAMPARI': {
        'L2': [0.99, 0.53, 0.72],
        'Cosine': [0.51, 0.86, 0.74]
    },
    'MLP Data': {
        'L2': [1.414, 1.448, 1.415],
        'Cosine': [0.0, -0.051, -0.002]
    },
    'Linear Data': {
...
for i, setting in enumerate(settings):
    l2_values = [data[dataset]['L2'][i+1] for dataset in datasets]  # i+1 to skip first index
    bars = ax1.bar(positions[i], l2_values, bar_width, 
                   label=setting, color=colors[i], alpha=0.8, edgecolor='white', linewidth=1)
    
    # Add text labels on bars
    for j, (bar, value) in enumerate(zip(bars, l2_values)):
...
for i, setting in enumerate(settings):
    ...
    bars = ax2.bar(positions[i], cosine_distance_values, bar_width, 
                   label=setting, color=colors[i], alpha=0.8, edgecolor='white', linewidth=1)
    
    # Add text labels on bars
    for j, (bar, value) in enumerate(zip(bars, cosine_distance_values)):
        height = bar.get_height()
