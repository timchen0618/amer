...
# =================================================================
seed=0
single_or_multi="multi_SS" # single, multi_SS, multi_sampling
...
doc_encoder="inf"
base_model_name="inf" # llama-1b, qwen3-4b, llama-3b, llama-8b
...
if [ "$single_or_multi" == "single" ]; then
    ...
    FORCE_SAMPLING=""
elif [ "$single_or_multi" == "multi_SS" ]; then
    mix_one_label_shuffled=true
    MODE="hungarian_contrastive"
    FORCE_SAMPLING=""
elif [ "$single_or_multi" == "multi_sampling" ]; then
    mix_one_label_shuffled=true
    MODE="hungarian_contrastive"
    FORCE_SAMPLING="--force_sampling"
else
    echo "Invalid single_or_multi"
    exit 1
fi
...
if [ "$MODEL_TYPE" == "EmbeddingModel" ]; then
    ...
    LEFT_PADDING="--left_padding"
    PRED_LENGTH="--pred_length"
elif [ "$MODEL_TYPE" == "EmbeddingModelSSVariableLeftPadDocEncTrained" ]; then
    MODEL_STR="_SSVariableLeftPadDocEncTrained"
    SCHEDULE_SAMPLING="--schedule_sampling"
    LEFT_PADDING="--left_padding"
    PRED_LENGTH=""
fi
...
if [ "$train_doc_encoder" = true ]; then
    TRAIN_DOC_ENCODER="--train_doc_encoder"
else
    TRAIN_DOC_ENCODER=""
fi
...
# Experiment prefix
# EXP_PREFIX="force_sampling_${mix_one_label_shuffled_prefix}${base_model_prefix}${normalize_prefix}ambiguous_qe${GPUS_PREFIX}${FINETUNING_STR}${MODEL_STR}_${MODE}"
# EXP_PREFIX="${base_model_name}_${single_or_multi}_${seed}"
if [ "$train_doc_encoder" = true ]; then
    EXP_PREFIX="${mix_one_label_shuffled_prefix}train_doc_encoder_${base_model_name}"
else
    EXP_PREFIX="${base_model_name}_${single_or_multi}_${seed}"
fi
...
if [ "$train_doc_encoder" = true ]; then
    BASE_SAVE_PATH="results/doc_enc_trained/${base_model_name}"
else
...
if [ "$train_doc_encoder" = true ]; then
    # BASE_TRAIN_PATH="training_datasets/doc_enc_trained/ambigqa_2_to_5_ctxs_sm"
    BASE_TRAIN_PATH="training_datasets/doc_enc_trained/autoregressive_ambiguous_qe_${doc_encoder}_train_dataset_contrastive_2_to_5_ctxs/"
else
