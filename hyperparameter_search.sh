...
singularity exec --nv --overlay \${OVERLAY_FILE}:ro \$SINGULARITY_IMAGE /bin/bash -c "source /ext3/env.sh; cd ${WORK_DIR}; (trap 'kill 0' SIGINT; HF_TOKEN=${HF_TOKEN} TORCH_DISTRIBUTED_DEBUG=INFO ${PYTHON_COMMAND} \$ARGS & wait)"
EOF
