...
if __name__ == "__main__":
    base_model = "llama-8b"
    data_type = "ambiguous_qe"
    
    # select_indices_file="/scratch/hc3337/projects/autoregressive/data/qampari_5_to_8/large_distance_indices_inf.txt"
    ...
    all_significantly_different = []
    for base_model in ["llama-1b", "llama-3b", "qwen3-4b", "llama-8b"]:
        for data_type in ["qampari", "ambiguous_qe"]:
            rootdir = Path(f'/scratch/hc3337/projects/autoregressive/results/{base_model}/{data_type}_inf/')
            for score_type in ["mrecall"]:
                print('-' * 40)
                print(f"Testing {score_type}")
                
                for seed_1, seed_2 in [(0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2), (1, 0), (2, 0), (2, 1)]:
                    data_file = f"retrieval_out_dev_{data_type}_5_to_8_max_new_tokens_1_{score_type}_topk100.npy" if data_type == "qampari" else f"retrieval_out_dev_ambiguous_qe_max_new_tokens_1_{score_type}_topk100.npy"
                    base_results = np.load(rootdir / f"{base_model}_single_{seed_1}" / data_file)

                    suffix_list = [
                        f"{base_model}_multi_SS_{seed_2}"
                    ]
                    ...
                    for suffix in suffix_list:
                        data_file = f"retrieval_out_dev_qampari_5_to_8_max_new_tokens_5_{score_type}_topk100.npy" if data_type == "qampari" else f"retrieval_out_dev_ambiguous_qe_max_new_tokens_2_{score_type}_topk100.npy"
                        results = np.load(rootdir / suffix / data_file)
                        print(len(results), len(base_results))
                        
                        if selected_indices is not None:
                            results = results[selected_indices]
                        sig_diff = is_significantly_different(base_results, results, verbose=True)
                        all_significantly_different.append(sig_diff)

    print(f"Significantly different: {np.mean(all_significantly_different)}")

    # print('======== AMBIGUOUS QE =========')
    # rootdir = Path('/scratch/hc3337/projects/autoregressive/results/qwen3-4b/ambiguous_qe_inf/')
    # # select_indices_file="/scratch/hc3337/projects/autoregressive/data/ambiguous/qampari_embeddings_data/large_distance_indices_inf.txt"
    # # selected_indices = [int(l.strip('\n').strip('\r')) for l in open(select_indices_file)]
    # selected_indices = None
    # score_type = "mrecall"
    # for score_type in ["mrecall"]:
