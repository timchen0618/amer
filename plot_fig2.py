            ...
            for distance_list in [cosine_distance_list]:
                ...
                for i in range(len(distance_list)):
                    if distance_list[i] < percentile_list[0]:
                        ...
                        all_bins[2].append(i)
                    # if distance_list[i] < 0.53:
                    #     all_bins[0].append(i)
                    # elif distance_list[i] < 0.63:
                    #     all_bins[1].append(i)
                    # elif distance_list[i] < 0.69:
                    #     all_bins[2].append(i)
                    else:
                        all_bins[-1].append(i)
...
if 'plot' in commands:
    ...
    for data_idx, data_name in enumerate(dataset_names):
        for length_idx in range(5):
            ...
            if length_idx == 0:
                ax.set_title(f'{dataset_labels[data_name]} (Overall)', fontsize=23, fontweight='bold', pad=15)
            else:
            ...
            ax.set_xticklabels([f'{k:.2f}' for k in percentile_list], fontsize=18, fontfamily='Arial')
