    ...
    # for data_type in ["qampari"]:
        ...
        # for model in ["qwen3-4b"]:
            for random_seed in [0, 1, 2]:
                print(f"Aggregating {data_type} with {model} and random seed {random_seed}")
                root_dir = f"results/{model}/{data_type}_inf/"
                results = read_jsonl(f"{root_dir}/{model}_single_{random_seed}/retrieval_out_dev_{data_type}_mmlf_max_new_tokens_1.jsonl")
                if data_type == "ambiguous_qe":
                    org_query_results = read_jsonl(f"{root_dir}/{model}_single_{random_seed}/retrieval_out_dev_{data_type}_max_new_tokens_1.jsonl")
                elif data_type == "qampari":
                    org_query_results = read_jsonl(f"{root_dir}/{model}_single_{random_seed}/retrieval_out_dev_{data_type}_5_to_8_max_new_tokens_1.jsonl")
                else:
                    raise ValueError(f"Invalid data type: {data_type}")
                ...
                write_jsonl(f"{root_dir}/{model}_single_{random_seed}/retrieval_out_dev_{data_type}_mmlf_max_new_tokens_1_aggregated.jsonl", out_results)